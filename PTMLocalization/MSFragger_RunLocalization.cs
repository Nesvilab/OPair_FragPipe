...
namespace PTMLocalization
{
    public class MSFragger_RunLocalization
    {
        ...
        public static readonly string OPAIR_HEADERS = "O-Pair Score\tNumber of Glycans\tTotal Glycan Composition\tGlycan Site Composition(s)\tComposition Delta Score\tNext Best Composition\tConfidence Level\tLocalization Delta Score\tSite Probabilities\t138/144 Ratio\tHas N-Glyc Sequon\tPaired Scan Num";
        ...
        public void LoadGlycanDatabase(double maxGlycanMass)
        {
            ...
            timer.Stop();
            Console.WriteLine("done in {0:0.0}s\n\tLoaded {1} glycans, {2} glycan combinations", timer.ElapsedMilliseconds * 0.001, GlycanBox.GlobalOGlycans.Length, GlycanBox.OGlycanBoxes.Length);
        }
        ...
        private MsDataFile CheckAndLoadData(string rawfileBase, string rawfileName, Dictionary<string, bool> mzmlNotFoundWarnings, bool usingLcmsFilePath)
        {
            ...
            else if (File.Exists(Path.Combine(rawfilesDirectory, rawfileBase + CAL_INPUT_EXTENSION_FALLBACK)))
            {
                ...
                {
                    Console.Write("\nError loading calibrated MGF file, file not loaded");
                    return null;
                }
            }
            else if (File.Exists(Path.Combine(rawfilesDirectory, rawfileBase + CAL_INPUT_EXTENSION_FALLBACK2)))
            {
                ...
                {
                    Console.Write("\nError loading uncalibrated MGF file, file not loaded");
                    return null;
                }
            }
        ...
        public GlycoSpectralMatch LocalizeOGlyc(Ms2ScanWithSpecificMass ms2Scan, PeptideWithSetModifications peptide, double glycanDeltaMass, double oxoRatio, Dictionary<FilterRule, bool> oxoniumBools)
        {
            ...
            gsm.DeltaScoreGraph = GlycoSpectralMatch.CalculateGraphDeltaScore(gsm, graphs);
